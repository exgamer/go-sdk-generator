...
func TestRun_GeneratesFilesInEmptyProject(t *testing.T) {
	...
	docsPath := filepath.Join(dir, "docs", "docs.go")

	mustParse(t, mainPath)
	mustParse(t, appPath)
	mustParse(t, docsPath)
	...
	if !strings.Contains(string(mainSrc), `_ "example.com/myservice/docs"`) {
		t.Errorf("main.go does not blank-import the docs package:\n%s", mainSrc)
	}

	docsSrc, err := os.ReadFile(docsPath)

	if err != nil {
		t.Fatal(err)
	}

	if !strings.Contains(string(docsSrc), "package docs") {
		t.Errorf("docs/docs.go does not declare package docs:\n%s", docsSrc)
	}
...
func TestRun_ForceNeverOverwritesDocsStub(t *testing.T) {
	dir := t.TempDir()
	writeGoMod(t, dir, "example.com/myservice")
	...
	realDocs := "package docs\n\n// generated by swag init, not our stub\n"
	...
	for _, f := range result.Files {
		if f.Path == filepath.Join("docs", "docs.go") && f.Written {
			t.Error("expected docs/docs.go to be skipped even with Force")
		}
	}
	...
	if string(src) != realDocs {
		t.Error("docs/docs.go content was overwritten by --force")
	}
}
